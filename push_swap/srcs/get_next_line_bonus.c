...
static t_line	*create_buff(t_line **head, int fd)
{
	t_line	*buff;

	buff = (t_line *)malloc(sizeof(t_line));
	if (!buff)
		return (NULL);
...
static int	expand_buff(t_line **head, t_line *buff, int fd)
{
	char	*n_str;
...
static char	*gen_line(t_line **head, t_line *buff, ssize_t i, int fd)
{
	char	*result;
...
static char	*find_line(t_line **head, t_line *buff, int fd)
{
	ssize_t	size;
	ssize_t	i;
